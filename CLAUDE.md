...
Release flow is `make preflight → make version-{patch|minor|major} → make tag →
make release`. Web Store submission is intentionally manual — no tag triggers
it. See `/ship` skill for the full runbook.
